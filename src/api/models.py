...
class Gasto(db.Model):
    ...
    # ✅ clave para proyecto final: gastos por usuario
    user_id: Mapped[int] = mapped_column(db.ForeignKey("user.id"), nullable=False, index=True)

    gasto: Mapped[str] = mapped_column(String(120), nullable=False)
...
class GroupMember(db.Model):
    ...
    def serialize(self):
        return {"id": self.id, "group_id": self.group_id, "user_id": self.user_id, "role": self.role}
